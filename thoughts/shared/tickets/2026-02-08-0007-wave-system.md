...
Relevant files:
- New domain module `src/gameplay/waves/mod.rs` — wave config data, WaveState resource (co-located)
- New systems in `src/gameplay/waves/`: `wave_spawner`, `wave_completion_check`
- Wave spawning uses `.in_set(GameSet::Production)`, wave UI uses `.in_set(GameSet::Ui)`
- Wave counter UI may live in `src/gameplay/waves/` or `src/screens/in_game.rs` (screen UI)
- Remove debug enemy spawner from `src/dev_tools/` (added in Ticket 4)
