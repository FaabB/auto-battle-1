...
Relevant files:
- `src/gameplay/units/mod.rs` — Health component (from Ticket 3), attack cooldown timer (co-located with unit domain)
- New domain module `src/gameplay/combat/mod.rs` or systems in `src/gameplay/units/`: `unit_attack`, `unit_death`, `health_bar_render`
- Combat systems use `.in_set(GameSet::Combat)`, death systems use `.in_set(GameSet::Death)`, health bar rendering uses `.in_set(GameSet::Ui)`
- Systems register in their own domain plugin, not in `screens/in_game.rs`
