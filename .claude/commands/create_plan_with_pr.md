...
2. Create worktree
3. Read ticket file + dependent tickets (ONLY ticket docs — no source files yet)
4. SPAWN 3+ research agents (pattern-finder, analyzer, locator) — WAIT for results
5. Read source files agents identified
6. Present approach to user — GET APPROVAL (gate)
7. Write plan
8. Quality review (no stream-of-consciousness, no incomplete snippets)
9. Commit, PR, Linear update
```

```
ticket → read ticket docs → agents (parallel) → read agent results → approach gate → plan → quality gate → commit/PR
                                                                           ↑
